...
def signup_user(data: SignupRequest, db: Session = Depends(get_db)):
    try:
        ...
        cognito_response = cognito_client.sign_up(
            ClientId=COGNITO_CLIENT_ID,  # ✔ correct now
            Username=data.email,
        ...
        if not cognito_sub:
            raise HTTPException(400, "Error during sign up")
        ...
        raise HTTPException(400, f"Cognito sign up error: {str(e)}")
...
def login_user(data: LoginRequest,response:Response):
    try:
       ...
       # return {"message": "Login successful", "cognito": cognito_response}
...
def refresh_token(
    ...
    try:
        if not refresh_token or not user_cognito_sub:
            raise HTTPException(400,"cookies cant be null")
        secret_hash = get_secret_hash(
            user_cognito_sub,
            COGNITO_CLIENT_ID,
            COGNITO_CLIENT_SECRET,
